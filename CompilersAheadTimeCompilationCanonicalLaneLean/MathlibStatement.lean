...
def mathlibProofObligation : MathlibProofObligation := {
  sourceKey := "compilers-aot-canonical-lane",
  theoremObject := "ConstrainedAOTClosure",
  commonCoreImported := true,
  theoremSpecificDefinitionsNative := true,
  theoremSpecificBridgeNative := true,
  theoremSpecificAdmittedClosureNative := true,
  unrestrictedClassicalClosureNative := false,
  carriedGap := "AOT-specific Mathlib endgame pilot closes over admitted class; unrestricted classical closure remains carried"
}
...
def theoremSpecificEndgamePilotClosed : Prop :=
  forall A : AdmissibleClass, ConstrainedAOTClosure A

theorem theorem_specific_endgame_pilot_checked :
    theoremSpecificEndgamePilotClosed := by
  intro A
  exact constrained_aot_endgame A
